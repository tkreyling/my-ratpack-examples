...
public class RatpackMongoClient {
    ...
    @Value
    public static class ExactlyOneElementExpected extends Error {
    }

    @Value
    public static class MongoException extends Error {
        Throwable throwable;
    }
    ...
    public static Promise<Validation<Error, Document>> findOne(MongoCollection<Document> collection, Bson filter) {
        return async(downstream -> collection.find(filter).into(new ArrayList<>(), (result, throwable) -> {
            if (throwable != null) {
                downstream.success(invalid(new MongoException(throwable)));
            } else {
                if (result.size() != 1) {
